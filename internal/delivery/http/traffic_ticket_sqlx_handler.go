...
func NewPostgresTrafficTicketSQLXRepository() *PostgresTrafficTicketSQLXRepository {
	return &PostgresTrafficTicketSQLXRepository{
		BaseMultiDBRepository: &BaseMultiDBRepository{
			dbs: initializeDatabasesTrafficPostgre(),
		},
	}
}

func initializeDatabasesTrafficPostgre() map[string]*sqlx.DB {
...
func (r *PostgresTrafficTicketSQLXRepository) GetPaginatedJSON_Traffic_Postgre(limit, offset int, dbName string) ([]byte, error) {
...
func (r *PostgresTrafficTicketSQLXRepository) InsertJSON_Traffic_Postgre(jsonData []byte, dbName string) error {
	ctx, cancel := context.WithTimeout(context.Background(), config.GetQueryTimeout())
	defer cancel()
	db := r.getDB(dbName)
