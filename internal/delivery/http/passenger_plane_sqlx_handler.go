...
func NewPassengerPlaneSQLXRepository() *PassengerPlaneSQLXRepository {
	return &PassengerPlaneSQLXRepository{
		BaseMultiDBRepository: &BaseMultiDBRepository{
			dbs: initializeDatabasesPassengerSQL(),
		},
	}
}
...
func (r *PassengerPlaneSQLXRepository) GetPaginatedJSON_Passenger_SQL(limit, offset int, dbName string) ([]byte, error) {
...
func (r *PassengerPlaneSQLXRepository) InsertJSON_Passenger_SQL(jsonData []byte, dbName string) error {
	ctx, cancel := context.WithTimeout(context.Background(), config.GetQueryTimeout())
	defer cancel()
	db := r.getDB(dbName)
